 ...
 */
function pickPlaceCellTowardTargetList(opts: {
  map: any;
  clanId: string;
  me: Pos;
  tx: number;
  ...
  /** Khoảng cách tối đa (cheby) tới cờ built đồng minh để cắm được. Game bắt kề sát (≤1). */
  maxFriendlyDist?: number;
  /** Loại trừ các ô đã thử bị reject (để "thử ô khác" tìm ô kế). */
  exclude?: Set<string>;
}): Pos[] {
  const { map, clanId, me, tx, ty, maxHop = 3, allowOnTarget = false, maxFriendlyDist = 3, exclude } = opts;
  const gridW = Math.max(20, Math.floor(n(map?.config?.grid_w, 85)));
  const gridH = Math.max(20, Math.floor(n(map?.config?.grid_h, 85)));
  const occ = occupiedCells(map);
  const flags = parseFlags(map);
    ...
    for (const a of anchors) {
      for (let dx = -maxHop; dx <= maxHop; dx++) {
        for (let dy = -maxHop; dy <= maxHop; dy++) {
          ...
          const fd = friendlyDist(x, y);
          if (fd > maxFriendlyDist) continue; // kề cờ built (cheby≤maxFriendlyDist)
  ...
  cands.sort((a, b) => a.score - b.score);
  return cands.map((c) => ({ x: c.x, y: c.y }));
}
...
function pickPlaceCellTowardTarget(opts: {
  ...
  const list = pickPlaceCellTowardTargetList(opts);
  return list[0] || null;
}
  ...
  async function runHoangCoCaptureSatellite(options: HoangCoAutoOptions, captureAll = false): Promise<HoangCoRunSummary | null> {
    ...
    try {
      ...
      const cand = satellites
        .filter((s: any) => String(s.holder_clan_id) !== clanId)
        .map((s: any) => {
          const sx = Math.floor(n(s.pos_x));
          const sy = Math.floor(n(s.pos_y));
          return { s, sx, sy, dFlag: nearestClanFlagDist(sx, sy), dMe: manhattan(me.x, me.y, sx, sy) };
        })
        .sort((a: any, b: any) => a.dFlag - b.dFlag || a.dMe - b.dMe)[0];
      if (!cand) return null;
      ...
      const touched = ownBuilt.some((f) => chebyshev(f.pos_x, f.pos_y, satTile.x, satTile.y) <= 2);
      ...
      // ── CHƯA có cờ clan BUILT trong cheby <= 2 của vệ tinh → BẮT BUỘC cắm bridge cho tới khi có.
      // Game chỉ cho công nếu có cờ đồng minh kề vệ tinh. Cắm từ cờ clan GẦN NHẤT (cheby <= 3 placement =
      ...
      if (!touched) {
        ...
        let lastCell: Pos | null = null;
        for (let attempt = 0; attempt < 8; attempt++) {
          // ưu tiên ô SÁT vệ tinh (cheby 1) nếu có thể neo từ cờ clan; nếu không, progress dần về phía sat.
          const cell = pickPlaceCellTowardTargetList({
          ...
          try {
            ...
            return base({ placed: 1, built: flagId ? 1 : 0, flagId, dest: { x: cell.x, y: cell.y }, action: "place_satellite_bridge", reason: `Vệ tinh ${cityKey}: cắm bridge #${flagId} @(${cell.x},${cell.y}) (cách ${cellToT}) · chờ cờ clan <=2` });
          } catch (pe: any) {
            ...
            if (isPlaceTooCloseToEnemyError(pe) || isNotAdjacentError(pe)) {
              excluded.add(cellKey(cell.x, cell.y));
              onLog?.("WARN", `Vệ tinh ${cityKey} · @(${cell.x},${cell.y}) lỗi · đổi ô (${String(pe?.message || "").slice(0, 60)})`);
              continue;
            }
            // lỗi khác (thường quá xa) → đi tới ô đó rồi đặt ở lượt sau
            const distPlace = manhattan(cell.x, cell.y, me.x, me.y);
            ...
            onLog?.("WARN", `Vệ tinh ${cityKey} · place lỗi: ${String(pe?.message || "").slice(0, 100)}`);
        ...
        return base({ reason: `Vệ tinh ${cityKey}: chưa có cờ clan <=2 · đang cắm bridge, chờ`, nextDelayMs: 8000 });
      }

      // ── ĐÃ có cờ clan BUILT trong cheby <= 2 → người chơi vào tầm công (cheby <= proximity) rồi CÔNG
